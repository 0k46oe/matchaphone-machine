...
export default function SocialLayout(){
 const loc=useLocation(),nav=useNavigate(),{messages,feedPosts}=useStore(),chatUnread=messages.filter(m=>m.origin==="proactive"&&!m.readAt).length,feedUnread=feedPosts.filter(p=>(p.origin==="proactive"&&!p.readAt)||p.comments.some(c=>c.origin==="proactive"&&!c.readAt)).length;
