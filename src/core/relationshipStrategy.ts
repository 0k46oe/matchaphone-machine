...
async function requestEvaluation(character:Character,userText:string,messages:Message[],characters:Character[],provider:ProviderSettings,signal?:AbortSignal){
 ...
 let last:unknown;
 for(let attempt=0;attempt<2;attempt++)try{const raw=await new OpenAIProvider(provider).chat([{role:"system",content:"你只负责关系变化评估，只输出严格 JSON。"},{role:"user",content:prompt}],{stream:false,signal});return validateStrategyEvaluation(parseJson(raw))}catch(error){last=error;if(error instanceof ProviderError&&error.kind==="aborted")throw error}
 throw last;
}
export async function evaluateStrategyInteraction(input:{character:Character;sourceId:string;userText:string;messages:Message[];characters:Character[];provider:ProviderSettings;signal?:AbortSignal}){
 ...
 const evaluation=await requestEvaluation(current,input.userText,input.messages,input.characters,input.provider,input.signal);
...
export async function generateConfessionMessages(input:{character:Character;context:ChatItem[];provider:ProviderSettings;signal?:AbortSignal}){
 ...
 const raw=await new OpenAIProvider(input.provider).chat([...input.context,request],{stream:false,signal:input.signal});return validateConfessionMessages(parseJson(raw));
}
